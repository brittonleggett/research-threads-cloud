...
## 2026-09-18 — nightly scouting run (WebSearch and WebFetch this session; WebFetch worked cleanly
against govtech.com but returned a 403 on sbcamericas.com, worked around via casino.org and WebSearch
snippets for that source instead of citing unread full text). Per this pass's scope, only this file
itself was read before starting (full header/section scan via grep across all 39 entries, plus a close
read of ideas 37-39 and the 2026-09-16/09-17 rejected-candidates lists) — not re-litigating idea 38's
now-flagged IJMR crowding or idea 37's standalone-vs-fold-in BESS question, per tonight's brief. Searched
AI-augmented regulatory/civic processes, gambling/betting policy, and Louisiana energy/infrastructure as
a cross-check against genre-redundancy before committing search time to the two candidates below.

### 40. AI-generated ("astroturfed") public comments as a threat to siting-decision legitimacy — a new
antecedent for the legitimacy/procedural-justice mediator chain already used by `DATA_CENTER_LEGITIMACY_
PAPER` and `CCS_PAPER`, not a restatement of either (high confidence — real, dated, multi-jurisdiction
2026 incidents with a confirmed regulatory-vote outcome; main caveat is no Louisiana-specific instance
was confirmed tonight, so the hook is national precedent transferable to Britton's own live dockets
rather than a Louisiana case in hand)

- **Gap/question:** `CCS_PAPER`'s own coded mediators (confirmed by this file's idea 37 entry via direct
  read of its Conceptual Model draft) are procedural justice (Tyler), institutional trust (Terwel et al.
  2009), and perceived legitimacy (Gehman et al. 2017) — all of which assume the public-comment/
  participation record being evaluated is genuine. 2026 has produced real, documented cases of that
  assumption breaking: AI tools can now generate thousands of individually-worded, plausible-sounding
  public comments attributed to real or fabricated residents, at a volume and sophistication classic
  astroturfing (form letters, bot networks) never reached. The open question: does *disclosure/awareness*
  that a siting-decision's public-comment record was substantially AI-astroturfed change a community
  member's perceived legitimacy of the resulting regulatory decision and downstream trust in the
  regulator/developer — as a mechanism distinct from classic procedural justice (which asks whether fair
  *procedures* were followed) because it instead asks whether the *input itself* was authentic. Call the
  new antecedent "process-integrity threat" — plausibly a stronger legitimacy-eroding shock than a
  procedural-justice violation alone, and plausibly moderated by whether the astroturfed outcome favored
  or opposed the respondent's own prior position (a motivated-reasoning test: does "my side won because of
  fake comments" get excused while "the other side won because of fake comments" gets condemned).
- **Why tractable soon:** confirmed via direct WebFetch of govtech.com (Feb 24, 2026) — the South Coast
  Air Quality Management District (SCAQMD, Southern California) voted 7-5 in June 2025 to reject proposed
  Rules 1111/1121 (fees on gas-powered furnaces/water heaters meant to cut NOx emissions across the
  region's ~10 million units), and public records later revealed (reported by the LA Times, per SCAQMD
  staff confirmation) that more than 20,000 of the opposition comments were generated by CiviClick, a
  Washington D.C.-based "AI-powered grassroots advocacy platform," with some named commenters reportedly
  unaware their names had been used. California lawmakers subsequently urged the state Attorney General to
  investigate (Sierra Club press release, March 2026). A separate, less-verified claim (via a secondary
  citation inside an arXiv trust-in-AI-disclosure paper, not independently confirmed tonight) alleges a
  similar AI-fabricated-comment pattern at the Bay Area Air Quality Management District — flagging as an
  unconfirmed lead, not a second confirmed case. This is squarely inside Britton's own corpus-source
  category (regulatory-agency public-comment dockets on environmental/infrastructure rules), and the
  mechanism is directly portable to Louisiana: the LPSC's own active data-center dockets (confirmed via
  WebSearch tonight — Docket X-37921, a large-load-guideline repository docket that took public comments
  through June 19, 2026, alongside the Meta Hyperion dockets U-37425/U-37882) are exactly the kind of
  record `DATA_CENTER_PAPER`/`DATA_CENTER_LEGITIMACY_PAPER` already treat as corpus, and exactly the kind
  of record this mechanism threatens — though no astroturfing has been alleged in any Louisiana docket as
  of tonight's search, an honest limit distinguishing this from an in-hand Louisiana case.
- **Saturation check, done directly:** pre-genAI astroturfing-and-trust research exists and is relevant
  background — "The Consequences of Astroturf Lobbying for Trust and Authenticity" (an experimental
  study, ResearchGate) and Walker & Le's "Poisoning the Well: How Astroturfing Harms Trust in Advocacy
  Organizations" (2023, *Nonprofit and Voluntary Sector Quarterly*) both confirm classic astroturfing
  erodes trust generically — but neither is AI-specific, neither is set in a local-infrastructure-siting-
  legitimacy context, and neither uses a mediated antecedent-mediator-outcome PLS-SEM design. The
  government-oversight/legal literature on regulatory comment-process integrity (ACUS's 2021 "Mass,
  Computer-Generated, and Fraudulent Comments" report) predates generative AI and is a legal/administrative
  analysis, not a consumer/citizen-psychology study. No PLS-SEM or experimental study was found testing
  whether *awareness of AI-astroturfed comments specifically* changes perceived legitimacy of a siting
  decision or downstream trust/opposition — this reads as a genuine, not just assumed, gap, and one that
  sits closer to Britton's own theoretical toolkit (Gehman-style legitimacy) than the general astroturfing
  literature does.
- **Honest caveat/overlap flag:** this sits close enough to `DATA_CENTER_LEGITIMACY_PAPER`'s own
  conceptual lane (legitimacy as central construct) and `CCS_PAPER`'s coded mediators that Britton should
  weigh whether this is best pursued as an extension/robustness-check antecedent within one of those
  already-active projects (does process-integrity threat predict incremental variance in legitimacy
  beyond procedural justice and institutional trust alone) rather than a fully separate paper — the same
  choice this file has flagged for ideas 28/12/34 relative to their nearest active projects. There is also
  a methodological angle worth flagging to whoever next works `DATA_CENTER_PAPER`/`CCS_PAPER` directly: if
  any of Britton's own Study 1 comment corpora were drawn from a docket later found to contain AI-
  astroturfed submissions, that is a corpus-integrity question for those projects, separate from whether
  this becomes its own paper.
- **Rough method sketch:** fits the antecedent→mediator→outcome template. Antecedent: disclosed/perceived
  AI-astroturfing of a siting decision's public-comment record (vignette manipulation modeled on the
  SCAQMD/CiviClick case, or measured awareness of a real local instance once/if one surfaces in a
  Louisiana docket). Mediator: perceived legitimacy of the resulting decision (Gehman et al.) and/or
  perceived authenticity of public participation, tested against procedural justice and institutional
  trust as comparison mediators to establish incremental contribution. Outcome: trust in the
  regulator/developer, support for the siting decision or its reversal, support for comment-authentication
  requirements (a live, named policy ask in the AI-astroturfing coverage). Plausible moderator: whether the
  astroturfed outcome aligned with the respondent's own prior position (motivated-reasoning test) or
  general institutional trust/political ideology.
- **Human-subjects flag:** Study 1 as scoped (public-record docket comments, news coverage, agency
  investigation records) is public-record content analysis, no IRB concern. Any Study 2 vignette-survey
  work measuring real respondents' legitimacy/trust judgments is primary human-subjects data and would
  need IRB approval before proceeding — flagging per standing repo rule, not proceeding past the idea
  stage.
- **Target venue:** *Journal of Public Policy & Marketing* (matches `DATA_CENTER_PAPER`/`DATA_CENTER_
  LEGITIMACY_PAPER`/`CCS_PAPER`'s own lane) as lead candidate; *Public Administration Review* as a strong
  alternate given the core question is about the integrity of a public-participation process rather than a
  brand/marketing relationship specifically.
- Leads: [GovTech — AI-Generated Comments Swayed California Air Decision (direct fetch)](https://www.govtech.com/artificial-intelligence/ai-generated-comments-swayed-california-air-decision), [Phys.org — Southern California air board rejected pollution rules after AI-generated flood of comments](https://phys.org/news/2026-02-southern-california-air-board-pollution.html), [Governing — AI-Generated Public Feedback Raises New Questions for Lawmakers](https://www.governing.com/artificial-intelligence/ai-generated-public-feedback-raises-new-questions-for-lawmakers), [Sierra Club — California lawmakers urge Attorney General to investigate AI astroturf campaign targeting clean air rules](https://www.sierraclub.org/press-releases/2026/03/california-lawmakers-urge-attorney-general-investigate-ai-astroturf-campaign), [TechCrunch, Sept 10 2026 — AI agents are flooding public services with new requests (broader "agentic flooding" context)](https://techcrunch.com/2026/09/10/ai-agents-are-flooding-public-services-with-new-requests/), [ACUS — Mass, Computer-Generated, and Fraudulent Comments (2021 draft report, pre-genAI background)](https://www.acus.gov/sites/default/files/documents/Mass,%20Computer-Generated,%20and%20Fraudulent%20Comments%20(Draft%20Report%204.2.21).pdf), [ResearchGate — The Consequences of Astroturf Lobbying for Trust and Authenticity](https://www.researchgate.net/publication/318731879_The_Consequences_of_Astroturf_Lobbying_for_Trust_and_Authenticity_Findings_and_Implications_from_an_Experiment), [SAGE — Poisoning the Well: How Astroturfing Harms Trust in Advocacy Organizations](https://journals.sagepub.com/doi/full/10.1177/23294965221123808)

Proposal only, per standing rule — nothing built, nothing committed to.

### 41. The gambling-loss "phantom income" tax cap (90% deduction limit, OBBBA) as a regulated-market
trust-erosion/channeling mechanism — distinct from `GAMBLING_SOCIAL_COST_PAPER`'s macro fiscal-dependence
design and from ideas 15/34's platform-level mechanisms (high confidence — real, dated, live congressional
fight with a committee vote literally yesterday; needs an explicit crowding caveat since this is the third
gambling-adjacent entry in this file)

- **Gap/question:** since the 2025 One Big Beautiful Bill Act (signed July 4, 2025), gambling losses have
  been deductible only up to 90% of losses against winnings starting tax year 2026 — meaning a bettor who
  wins $100,000 and loses $100,000 across a year still owes federal tax on $10,000 of "phantom income"
  they never actually kept, a framing the industry and Nevada's congressional delegation have used
  specifically to argue the policy is *perceived as unfair* in a way ordinary tax complexity isn't. The
  open marketing question, distinct from ideas 15 (regulatory-classification loophole letting prediction
  markets dodge gambling ad rules) and 34 (platform-level algorithmic targeting of vulnerable bettors):
  does perceived unfairness of this specific tax-policy asymmetry (taxed on money never kept) erode trust
  in the *regulated/legal* betting system itself — as opposed to trust in any individual sportsbook — and
  does that trust erosion predict a stated willingness to shift action to offshore/unregulated books
  (which don't report winnings and offer no consumer protections), the exact "channeling" failure mode
  legal sports-betting regulation was designed to prevent in the first place.
- **Why tractable soon:** confirmed via direct WebFetch (casino.org) and WebSearch corroboration
  (Las Vegas Sun, Covers.com, Paulick Report, Yogonet, all Sept 16-17, 2026) — the House Ways and Means
  Committee voted 38-5 on September 16, 2026 to advance H.R. 10357 (the Digital Asset Tax Certainty Act,
  which absorbed the bipartisan FULL HOUSE Act's language, Rep. Steven Horsford D-NV and Rep. Max Miller
  R-OH, introduced January 2026) to restore the full 100% deduction, with a companion Senate-side FAIR BET
  Act pushed by Sen. Catherine Cortez Masto and Rep. Dina Titus. Lawmakers are racing to act before the
  90% cap "locks in" for the 2026 tax year when returns are filed starting January 1, 2027; the repeal is
  projected to cost roughly $2B in federal revenue over 2027-2036. Nevada's delegation and gaming-state
  representatives explicitly warned in committee that the cap "would have devastating consequences on the
  legal gaming market and push some to offshore gambling," naming the channeling risk directly as their
  argument, not something this idea is inferring. This is a live, resolving-in-real-time policy fight (full
  House and Senate votes still pending as of tonight), not a retrospective one.
- **Saturation check, done directly:** found real, relevant-but-distinct literature. Macro/economic
  channeling-and-taxation research exists (a Harm Reduction Journal paper on channelling and taxation in
  European online gambling markets; a PwC study linking higher gambling taxes to black-market expansion)
  but this is market-level econometric work, not individual-level psychological-mediator survey research.
  Tax-fairness-perception-and-compliance PLS-SEM literature exists in general tax contexts (not gambling-
  specific). A 2018 "Fair and easy" paper (*International Gambling Studies*) studies perceived fairness of
  slot-machine game mechanics and gambling intention — a different fairness target (game-outcome fairness,
  not tax-policy fairness) entirely. No PLS-SEM or experimental study was found testing whether perceived
  unfairness of *this specific tax-policy asymmetry* predicts trust in the regulated market or stated
  offshore-switching intention — this reads as a genuine gap, not an already-answered question.
- **Honest crowding caveat:** this is the third gambling-adjacent entry logged in this file (after idea 15's
  prediction-market classification loophole and idea 34's algorithmic-targeting trust mechanism), and idea
  34 already carries its own honest overlap flag against `GAMBLING_SOCIAL_COST_PAPER`'s planned Study 2.
  Britton should weigh whether a fourth gambling angle overall is additive to that project or whether this
  reads better as a policy-environment moderator folded into `GAMBLING_SOCIAL_COST_PAPER`'s existing macro
  fiscal-dependence design (state-level tax/regulatory-stringency variation already appears to be part of
  that project's Study 1) rather than a standalone paper. The differentiator, if pursued separately: this
  mechanism is about trust in the *regulatory system itself* (federal tax treatment) driving a channeling
  outcome, not about platform behavior or ad-classification loopholes, which is a genuinely distinct
  "wronged party" and causal story from ideas 15 and 34.
- **Rough method sketch:** antecedent = disclosed/described "phantom income" tax framing (vignette:
  bettor breaks even but owes tax on 10% of gross losses, modeled directly on the real $100k/$100k example
  used throughout press coverage) vs. a control condition describing ordinary tax filing. Mediator =
  perceived fairness/betrayal by the regulated system (distinct from trust in any specific operator).
  Outcome = stated willingness to consider offshore/unregulated betting, support for repeal (FULL HOUSE/
  FAIR BET Act-style), continued legal-market play intention. Plausible moderator: bettor volume/frequency
  (does the effect scale with how exposed a respondent actually is to the tax change) or prior awareness of
  the policy.
- **Human-subjects flag:** Study 1 as scoped (legislative text, committee testimony, press coverage) is
  public-record content analysis, no IRB concern. Any Study 2 survey/vignette work measuring real bettors'
  fairness/trust/switching judgments is primary human-subjects data and would need IRB approval before
  proceeding — flagging per standing repo rule, not proceeding past the idea stage.
- **Target venue:** *Journal of Public Policy & Marketing* (matches `GAMBLING_SOCIAL_COST_PAPER`'s own
  target) as lead candidate; *Journal of Consumer Affairs* or *Journal of Gambling Studies* as alternates.
- Leads: [Casino.org — Is Restoring the Deduction for Gambling Losses That Big of a Deal? Here's What the IRS Says (direct fetch)](https://www.casino.org/news/is-restoring-deduction-gambling-losses-that-big-of-deal/), [Las Vegas Sun — House Ways and Means Committee advances bill to restore full gambling loss tax deductions](https://lasvegassun.com/news/2026/sep/17/house-ways-and-means-committee-advances-bill-to-re/), [Covers.com — House Vote Advances Repeal of 90% Gambling Loss Deduction Limit](https://www.covers.com/industry/house-committee-advances-repeal-of-90-percent-gambling-loss-deduction-limit-sept-16-2026), [Casino.org — Odds Improve That Federal Tax Deduction for Gambling Losses Will Be Restored to 100%](https://www.casino.org/news/odds-improve-tax-deduction-gambling-losses-will-be-restored/), [Forbes, Jan 13 2026 — Gambling Tax Alert: New Law Cuts Loss Deductions, Bettors Face Big Hit](https://www.forbes.com/sites/nathangoldman/2026/01/13/gambling-tax-alert-new-law-cuts-loss-deductions-bettors-face-big-hit/), [Springer/Harm Reduction Journal — Channelling and taxation in European online gambling markets](https://link.springer.com/article/10.1186/s12954-024-01145-0), [iGamingToday — PwC Study Reveals Link Between Increased Gambling Taxes and Black Market Expansion](https://www.igamingtoday.com/pwc-study-reveals-link-between-increased-gambling-taxes-and-black-market-expansion/)

Proposal only, per standing rule — nothing built, nothing committed to.

Also checked tonight and explicitly set aside, each for a specific reason:
- **CF Industries/JERA/Mitsui's $4B ammonia plant (Ascension Parish, LA)** — real and current (a
  "recently reached key financial milestone" story, per WebSearch tonight), sited less than 8 miles from
  Air Products' now-cancelled blue-hydrogen project in the same industrial corridor, but no opposition
  angle was found tonight (positive financing-milestone coverage only) — nothing to write up as a siting-
  opposition entry yet, and this genre is already heavily saturated per this file's repeated prior
  rule-outs (most recently idea 37's BESS entry and the 2026-09-16/09-17 warehouse/semiconductor/crypto-
  mining rule-outs). Worth another look only if resident opposition actually surfaces.
- **A possible Bay Area Air Quality Management District AI-fabricated-comment case**, referenced only as
  a secondary citation inside an unrelated arXiv paper tonight, not independently confirmed via a primary
  or news source — folded into idea 40 above as an explicitly unconfirmed lead rather than logged as its
  own corroborating case.

Ideas 1-39 were not individually rechecked for fresh developments tonight beyond the crowding/overlap
