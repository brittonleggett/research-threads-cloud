...
5. "This is descriptive/correlational, not causal, for the policy-regime design (C/E)" — be upfront in the manuscript about which parts of the design are causal (harm outcomes via DiD) versus associational (policy-regime comparison), rather than overclaiming causal language for the political-economy mechanism. **2026-09-15 update**: `research_design/identification_strategy.md` now cites a real, verified, structurally-analogous precedent for exactly this defense — Jayawardhana et al. (2014, PLoS ONE) study tobacco Master Settlement Agreement revenue dependence predicting weaker state tobacco-control regulation using plain fixed-effects panel regression with an explicitly acknowledged, unresolved endogeneity caveat, no IV. It's a peer-reviewed publication precedent for reporting this exact class of dependence-predicts-policy-laxity relationship as an honest association rather than forcing a causal claim — useful ammunition for the reviewer-response letter if this criticism comes up, not a full identification-strategy solution (that choice is still Britton's).
